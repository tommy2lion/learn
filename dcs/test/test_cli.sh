...
# 13. R-15: --version exits 0 and prints "DCS <semver>:<date>:<commit>:<sig>"
#         All four fields auto-generated by the Makefile. Sig is 64 hex
#         chars (SHA-256). Commit may be "unknown" if not in a git tree.
out=$($EXE --version | tr -d '\r')
if echo "$out" | grep -Eq '^DCS [0-9]+\.[0-9]+\.[0-9]+:[0-9]{8}:([0-9a-f]{8}|unknown):[0-9a-f]{64}$'; then
    printf "PASS  --version: matches 'DCS X.Y.Z:YYYYMMDD:<commit>:<sig>' shape\n"
    PASS=$((PASS + 1))
else

...
# 13. R-15: --version exits 0 and prints "DCS <semver>:<date>:<commit>"
#         (date and commit are auto-generated by the Makefile; regex
#         accepts any 8-digit date and any 8-char hex commit OR "unknown"
#         when not in a git working tree)
out=$($EXE --version | tr -d '\r')
if echo "$out" | grep -Eq '^DCS [0-9]+\.[0-9]+\.[0-9]+:[0-9]{8}:([0-9a-f]{8}|unknown)$'; then
    printf "PASS  --version: matches 'DCS X.Y.Z:YYYYMMDD:<commit>' shape\n"
    PASS=$((PASS + 1))
else
